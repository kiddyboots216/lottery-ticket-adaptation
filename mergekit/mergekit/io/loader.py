...
class DumbPytorchLoader(TensorLoader):
    ...
    def get_tensor(self, key: str) -> torch.Tensor:
        ...
        # Check if the key without the first 6 characters exists
        elif key[6:] in self.tensors:
            return self.tensors[key[6:]]
        # If the key does not exist, attempt to access "model.{key}"
        elif f"model.{key}" in self.tensors:
            return self.tensors[f"model.{key}"]
